...
subplots_fig.suptitle("LMBGE Gradient Estimation - SP-BFGS", fontsize=14)
...
@printf("====Table Values====\n")
@printf("SP-BFGS GE Mean Number Of Objective Evaluations: %d\n", mean(obj_fun_eval_counts))
@printf("SP-BFGS GE Median Number Of Objective Evaluations: %d\n", median(obj_fun_eval_counts))
@printf("SP-BFGS GE Mean Log10 Optimality Gap: %1.1E\n", mean(log.(10,all_true_obj_fun_vals[end,:].-obj_fun_min_val)))
@printf("SP-BFGS GE Median Log10 Optimality Gap: %1.1E\n", median(log.(10,all_true_obj_fun_vals[end,:].-obj_fun_min_val)))
