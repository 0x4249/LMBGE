...
subplots_fig.suptitle("LMBGE Joint Function And Gradient Estimation - BFGS", fontsize=14)
...
for r in 1:total_runs
    ...
    axes_opt_gap.plot(0:pltIter,log.(10,all_true_obj_fun_vals[:,r]), alpha=0.8, color=color_map(r/total_runs), label="BFGS")
end
...
for r in 1:total_runs
    axes_sigma.plot(0:pltIter,log.(10,all_sigma_est_vals[:,r]), alpha=0.8, color=color_map(r/total_runs), label="BFGS")
end
...
for r in 1:total_runs
    ...
    axes_grad_norm.plot(0:pltIter,log.(10,all_grad_norms[:,r]), alpha=0.8, color=color_map(r/total_runs), label="BFGS")
end
...
for r in 1:total_runs
    axes_grad_est_norm.plot(0:pltIter,log.(10,all_grad_est_norm_vals[:,r]), alpha=0.8, color=color_map(r/total_runs), label="BFGS")
end

# Print table statistics
@printf("====Table Values====\n")
@printf("BFGS F&GE Mean Number Of Objective Evaluations: %d\n", mean(obj_fun_eval_counts))
@printf("BFGS F&GE Median Number Of Objective Evaluations: %d\n", median(obj_fun_eval_counts))
@printf("BFGS F&GE Mean Log10 Optimality Gap: %1.1E\n", mean(log.(10,all_true_obj_fun_vals[end,:].-obj_fun_min_val)))
@printf("BFGS F&GE Median Log10 Optimality Gap: %1.1E\n", median(log.(10,all_true_obj_fun_vals[end,:].-obj_fun_min_val)))
